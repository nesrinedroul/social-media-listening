...
const rolePill: Record<Role, { bg: string; color: string }> = {
  admin:      { bg: '#4c1d9522', color: '#a78bfa' },
  supervisor: { bg: '#1e3a8a22', color: '#60a5fa' },
  agent:      { bg: 'var(--active)', color: 'var(--text-2)' },
};
...
export function UsersPage() {
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<FormData>({
    resolver: zodResolver(schema), defaultValues: { role: 'agent' },
  });

  const createMutation = useMutation({
    mutationFn: (data: FormData) => authApi.register({ ...data, first_name: data.first_name ?? '', last_name: data.last_name ?? '' }),
    ...
    <div className="flex flex-col h-full" style={{ background: 'var(--page)' }}>
      <div className="px-5 py-3" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center justify-between mb-3">
          ...
          <Button size="sm" style={{ background: 'var(--brand)', color: '#fff' }} onClick={() => setAddOpen(true)}>
        ...
        <div className="flex gap-2">
          ...
          <select value={roleFilter} onChange={e => setRoleFilter(e.target.value)}
            style={{ background: 'var(--input-bg)', border: '1px solid var(--border)', borderRadius: 8,
              padding: '6px 8px', fontSize: 13, color: 'var(--text-2)', outline: 'none' }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  ...
                  <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full"
                    style={rp}>{u.role}</span>
                  {!u.is_active && (
                    <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full"
                      style={{ background: '#7f1d1d22', color: '#f87171' }}>Inactive</span>
                  )}
                </div>
                <p className="text-xs truncate" style={{ color: 'var(--text-3)' }}>{u.email}</p>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-xs" style={{ color: 'var(--text-3)' }}>Joined {timeAgo(u.created_at)}</span>
                {u.is_active && (
      ...
      <Modal open={addOpen} onClose={() => { setAddOpen(false); reset(); }} title="Add new user">
        <form onSubmit={handleSubmit(d => createMutation.mutate(d))} className="space-y-3">
          ...
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium uppercase tracking-wide" style={{ color: 'var(--text-2)' }}>Role</label>
          ...
          {createMutation.isError && (
            <p className="text-xs text-red-400 rounded px-3 py-2"
              style={{ background: '#ef444415', border: '1px solid #ef444430' }}>
              Failed to create user. Check inputs and try again.
            </p>
          )}
          <div className="flex justify-end gap-2 pt-1">
            <Button variant="secondary" size="sm" type="button" onClick={() => { setAddOpen(false); reset(); }}>Cancel</Button>
