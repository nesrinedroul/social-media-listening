...
const statusDotColor = { online: 'bg-emerald-500', busy: 'bg-amber-500', offline: 'bg-slate-500' };
...
export function Avatar({ name, size = 'md', status }: AvatarProps) {
  ...
  return (
    <div className="relative shrink-0">
      ...
      {status && (
        <span className={`absolute -bottom-0.5 -right-0.5 ${dotMap[size]} ${statusDotColor[status]} rounded-full ring-2 ring-slate-900`} />
      )}
    </div>
  );
}